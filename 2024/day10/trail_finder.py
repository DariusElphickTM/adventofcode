...
class TrailFinder():
    ...
    def dfs_recurse(self, adjacency_matrix, visited_peaks, current_vertex, allow_multiple_visits_to_peak):
        ...
        if self.trail_positions[current_vertex] == '9':
            #print("Reached a 9!")
            if allow_multiple_visits_to_peak or not current_vertex in visited_peaks:
                current_trail_score += 1
                visited_peaks.append(current_vertex)
        
        for i, weight in enumerate(adjacency_matrix[current_vertex]):
            if weight == 1:
                current_trail_score += self.dfs_recurse(adjacency_matrix, visited_peaks, i, allow_multiple_visits_to_peak)
        return current_trail_score
    
    def perform_depth_first_search(self, starting_vertex, allow_multiple_visits_to_peak = False):
        visited_peaks = []
        return self.dfs_recurse(self.trail_map, visited_peaks, starting_vertex, allow_multiple_visits_to_peak)
    ...
    def get_trailhead_rating(self, start_vertex):
        return self.perform_depth_first_search(start_vertex, True)
    ...
    def get_rating_for_all_trailheads(self):
        trail_head_indexes = self.get_trailhead_indexes()
        total_rating = 0
        for trail_head_index in trail_head_indexes:
            total_rating += self.get_trailhead_rating(trail_head_index)
        return total_rating
