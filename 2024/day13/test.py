import unittest
from claw_machine_player import ComputationalClawMachinePlayer, TreeNode, MathematicalClawMachinePlayer
...
class TestMathematicalClawMachinePlayer(unittest.TestCase):
    example_inputs = [
        {
            'string': """Button A: X+94, Y+34
Button B: X+22, Y+67
...
Prize: X=5, Y=4""",
    ...
    def test_parses_input_successfully(self):
        test_input = self.example_inputs[0]
        test_player = MathematicalClawMachinePlayer(test_input['string'])
        self.assertDictEqual({
            'x': 94,
            'y': 34
        }, test_player.a_button_action)
